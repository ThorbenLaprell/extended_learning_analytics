...
abstract class report_preview {
    ...
    public abstract static function content(): array; // returns array with content to show

    public static function box(string $titlekey, string $titletext, string $icon, $subtext, $value, int $diff, string $report = null) {
        $difftriangle = '';
        $difftext = $diff;
        ...
        return self::boxcomplex($titlekey, $titletext, $icon, $subtext, $value, "{$difftriangle} {$difftext}", $report);
    }

    public static function boxcomplex(string $titlekey, string $titletext, string $icon, string $subtext, $maintext, $change, string $report = null) {
        $titlestr = $titletext;
        if ($report !== null) {
            $link = new \moodle_url("/local/learning_analytics/index.php/reports/{$report}");
